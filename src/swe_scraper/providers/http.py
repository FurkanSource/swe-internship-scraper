...
class RequestsJsonClient:
    ...
    def _decode(self, response: requests.Response) -> Any:
        ...
        if len(response.content) > self.max_response_bytes:
            raise ValueError(f"JSON response exceeded {self.max_response_bytes} bytes")
        content_type = response.headers.get("Content-Type", "").casefold()
        if "json" not in content_type and response.content[:1] not in {b"{", b"["}:
            raise ValueError(
    ...
    def _bounded_content(self, response: requests.Response) -> bytes:
        ...
        if len(content) > self.max_response_bytes:
            raise ValueError(f"HTTP response exceeded {self.max_response_bytes} bytes")
        return content
